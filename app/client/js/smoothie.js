class Smoothie {
	constructor(
		options = {
			...
			fastupdate: undefined, // provede se každej requestAnimate
	...
	togglePause() {
		...
		console.warn("Smoothie pause:", this.paused);
	}
	...
	gameLoop(timestamp) {
		...
		if (!this.paused) {
			...
			let interpolate = () => {
				...
				this.elapsed = current - this._startTime;

				//Catch any unexpectedly large frame rate spikes
				if (this.elapsed > 1000) this.elapsed = this._frameDuration;

				//For interpolation:
				this._startTime = current;

				//Add the elapsed time to the lag counter
				this._lag += this.elapsed;
				...
				if(this.waitOnNewFrame){
					this.waitOnNewFrame();
					this.waitOnNewFrame = null;
				}
			...
			let interpolateSprite = (sprite) => {
				...
				if (this.properties.position) {
					...
					if (sprite._previousX !== undefined) {
						...
						// if(sprite.antDesu || sprite.kurwa)
							// console.log(sprite._previousX, sprite._currentX, sprite.x);
					}
