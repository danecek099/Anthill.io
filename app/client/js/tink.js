class Tink {
  ...
  makePointer(element = this.element, scale = this.scale) {
    ...
    let pointer = {
      ...
      downHandler(event) {
        if (event.button == 2) {
          ...
          this.isUp = false;
          if (this.rightPress) this.rightPress();
        } else {
          //Set the down states
          this.isDown = true;
          this.isUp = false;
    ...
    element.addEventListener("contextmenu", pointer.rightDownHandler.bind(pointer), false);
    ...
    // když vyjede z okna
    element.addEventListener(
      "mouseleave", pointer.mouseLeaveHandler.bind(pointer), false
    );
    // když se tam vrátí
    element.addEventListener(
      "mouseenter", pointer.mouseEnterHandler.bind(pointer), false
    );
    ...
    this.pointers.forEach(pointer => {
      ...
      if (pointer.isDown) {
        ...
        //position, with the calculated offset
          ...
          pointer.dragSprite.x = pointer.x - pointer.dragOffsetX - pointer.dragSprite.parent.parent.transform.position.x;
          pointer.dragSprite.y = pointer.y - pointer.dragOffsetY - pointer.dragSprite.parent.parent.transform.position.y;
        }
      }
  ...
  makeInteractive(o) {
    ...
    o.rightPress = o.rightPress || undefined;
    ...
    this.pointers.forEach(pointer => {
      ...
      this.buttons.forEach(o => {
        ...
        if (o.enabled) {
          ...
          if (o.state === "rightDown") {
            if (!o.pressed) {
              if (o.rightPress) o.rightPress();
              o.pressed = false;
              o.action = "pressed";
            }
          }
  ...
  update() {
    ...
    //}
    //Moje dragSelect metoda
    //if (this.pointers[0].dragSprite === null && this.pointers[0].isDown) this.dragSelectUpdate();
  }
  ...
  */
  keyboard(keyCode) {
    ...
    key.disable = () => key.enabled = false;

    key.enable = () => key.enabled = true;
